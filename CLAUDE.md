...
Design priorities, in order: low cost, few moving parts, reproducibility per env. Trade-offs are documented in `README.md` ("Trade-offs" section) — respect them before adding deps or services.
...
Apply DAG: `cluster` → (`acme`, `cloudflare-dns` in parallel) → `argocd`.
